...
docker run -u $(id -u ${USER}):$(id -g ${USER}) --name data_backup -v bitbucket_data:/bitbucket_data \
-v $(pwd):/backup alpine \
sh -c "cd bitbucket_data && tar czf /backup/${DATE}-bitbucket.tar.gz ." > /dev/null 2>&1
